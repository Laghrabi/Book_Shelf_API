...
def handle_user_books():
    ...
    if request.method == 'POST':
        ...
        if storage.get_specific(UserBook, 'book_id', book.id):
            return jsonify({"error": "User Book relationship already exists"}), 400
...
@app_views.route('/user_books/<user_book_id>', methods=['GET', 'PUT', 'DELETE'])
@jwt_required()
def handle_user_book(user_book_id):
    current_user = isAuthenticated()
    user_book = storage.get_specific(UserBook, 'id', user_book_id)
    if not user_book:
        return jsonify({"error": "User Book relationship not found"}), 404
